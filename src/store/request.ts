...
connect(_client);
export function media_response_format<T>(r: Result<{ code: number | string; msg: string; data: T }>) {
  if (r.error) {
...
export const client: HttpClientCore = {
  ...
  async get<T>(...args: Parameters<typeof _client.get>) {
    const r = await _client.get<{ code: number | string; msg: string; data: T }>(...args);
    return media_response_format<T>(r);
  },
  async post<T>(...args: Parameters<typeof _client.post>) {
    const r = await _client.post<{ code: number | string; msg: string; data: T }>(...args);
    return media_response_format(r);
  },
