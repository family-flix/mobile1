...
export class TV {
  ...
  onErrorNotice: (msg: string) => void = noop;
  onNotice: (msg: string) => void = noop;
  ...
  async init(id: string) {
    ...
    const { name, first_episode } = resp.data;
    ...
    if (history_resp.data) {
      ...
      const r = await this.play_episode(episode_id);
      if (r.error) {
    ...
    const r = await this.play_episode(first_episode.id);
    if (r.error) {
      return Result.Err(r.error);
    }
    return Result.Ok(null);
  ...
  async play_episode(episode_id: string) {
    if (episode_id === this._cur_episode?.id) {
      this.notice(`${this._info?.name} 第 ${this._cur_episode.episode} 集`);
      return Result.Ok(this._cur_episode);
    }
  ...
  error_notice(res: Result<unknown>) {
    if (!res.error) {
      const e = "未知错误";
      this.onErrorNotice(e);
      return Result.Err(e);
    }
    this.onErrorNotice(res.error.message);
    return res;
  }
  notice(msg: string) {
    this.onNotice(msg);
